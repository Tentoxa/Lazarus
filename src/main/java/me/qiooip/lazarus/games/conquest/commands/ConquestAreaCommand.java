...
public class ConquestAreaCommand extends SubCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if(selection == null || selection.getType() != SelectionType.SYSTEM_CLAIM) {
            ...
            player.sendMessage(Language.CONQUEST_PREFIX + Language.CONQUEST_AREA_MAKE_A_SELECTION);
            return;
        }

        if(!selection.areBothPositionsSet()) {
            player.sendMessage(Language.CONQUEST_PREFIX + Language.CONQUEST_AREA_SET_BOTH_POSITIONS);
            return;
        }

        if(!ClaimManager.getInstance().getClaimsInSelection(selection.getPosOne(), selection.getPosTwo()).isEmpty()) {
            player.sendMessage(Language.CONQUEST_PREFIX + Language.CONQUEST_AREA_CLAIM_OVERLAPPING);
            return;
        }
        ...
        Messages.sendMessage(Language.CONQUEST_PREFIX + Language.CONQUEST_AREA_CREATED
