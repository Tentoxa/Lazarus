...
public class SetDtrCommand extends SubCommand {

    public SetDtrCommand() {
        super("setdtr", Collections.singletonList("dtrset"), "lazarus.factions.dtr");
    }

    @Override
    public void execute(CommandSender sender, String[] args) {
        if(args.length < 2) {
            sender.sendMessage(Language.FACTION_PREFIX + Language.FACTIONS_SET_DTR_USAGE);
            return;
        }
